...
func (txn *MvccTxn) GetLock(key []byte) (*Lock, error) {
	// Your Code Here (4A).
	l, err := txn.Reader.GetCF(engine_util.CfLock, key)
	if err != nil {
		return nil, err
	}
	if l == nil || len(l) == 0 {
...
func (txn *MvccTxn) CurrentWrite(key []byte) (*Write, uint64, error) {
	...
	it := txn.Reader.IterCF(engine_util.CfWrite)
	defer it.Close()
	for it.Seek(EncodeKey(key, math.MaxUint64)); it.Valid(); it.Next() {
		if bytes.Equal(DecodeUserKey(it.Item().Key()), key) == false {
			break
		}
		w, err := it.Item().Value()
		if err != nil {
		...
		write, err := ParseWrite(w)
		if err != nil {
			continue
		}
		if write.StartTS == txn.StartTS {
			ts := decodeTimestamp(it.Item().Key())
			return write, ts, nil
		}
	}
	return nil, 0, nil
